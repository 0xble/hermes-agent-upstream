...
def test_history_to_messages_ships_full_tool_args():
    ...
    # collapsed row titles, and renderers that show the full call (the
    # desktop's expanded `$` transcript) rebuild it from `args`. Dropping
    # args here made the preview truncation permanent after hydration.
...
def test_tool_start_ships_full_args(monkeypatch):
    # Desktop rebuilds the expanded row's `$` transcript from args; sending
    # only the 80-char `context` preview left the expanded command truncated
    # until tool.complete. tool.complete already ships full args to every
    # client, so tool.start does too — no per-client gate.
    events: list[tuple[str, str, dict]] = []
    monkeypatch.setattr(
        server, "_emit", lambda event_type, sid, payload: events.append((event_type, sid, payload))
    )
    ...
    # Empty args stay omitted — no noise key on argless tools.
