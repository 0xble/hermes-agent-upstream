...
_current_run: ContextVar[CronMessagingRun | None] = ContextVar("cron_messaging_run", default=None)


def current_run() -> CronMessagingRun | None:
    from agent.delegation_context import is_delegated_child_context

    run = _current_run.get()
    if run is None or not run.active or is_delegated_child_context():
        return None
    return run
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    columns = {row[1] for row in conn.execute("PRAGMA table_info(outbound_messages)")}
    if "attempt" not in columns:
        conn.execute("ALTER TABLE outbound_messages ADD COLUMN attempt INTEGER NOT NULL DEFAULT 1")
...
def job_allows_messaging(job: Optional[Dict[str, Any]]) -> bool:
    if not isinstance(job, dict):
        return False
    return job.get("allow_messaging") is True
...
def current_cron_job_id() -> str:
    run = current_run()
    return run.job_id if run else ""


def is_cron_messaging_session() -> bool:
    run = current_run()
    return bool(run and run.allowed and run.origin)
    ...
    with _transaction() as conn:
        ...
        if existing:
            ...
            if (record["body_hash"] != body_hash or record["target"] != target
                    or (record["platform"], record["chat_id"], record["thread_id"])
            ...
            if record["status"] == "failed":
                conn.execute(
                    """UPDATE outbound_messages
                       SET status='queued', error=NULL, updated_at=?, attempt=attempt+1
    ...
    with _transaction() as conn:
        conn.execute(
            """UPDATE outbound_messages
               ...
               WHERE job_id=? AND run_id=? AND message_key=? AND status='queued' AND attempt=?""",
            (
                status,
...
def classify_send_result(result: Any) -> Dict[str, Any]:
    ...
    if result.get("error"):
        return {
            "status": "failed" if result.get("delivery_not_attempted") is True else "ambiguous",
            "transport_message_id": result.get("message_id"),
            "error": ("send engine confirmed no delivery attempt"
                      if result.get("delivery_not_attempted") is True
                      else "send engine did not confirm delivery"),
        }
