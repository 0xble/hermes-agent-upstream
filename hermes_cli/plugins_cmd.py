...
def _install_plugin_core(
    ...
    *,
    ...
    The cloned tree is security-scanned in its temporary location before
    being moved into the plugins directory (see ``tools/plugin_guard.py``;
    inspired by Claude Cowork's skill & plugin scanning). ``scan_decision_cb``
    is called with the ScanResult for caution verdicts and may return True
    to accept the risk interactively.

    Returns ``(target_dir, installed_manifest, canonical_name)``.
    Raises ``PluginOperationError`` on failure (``PluginScanBlocked`` when
    blocked by the scanner).
    """
    import tempfile
